# ADR-0011: Claude-only for the development cycle — OAuth, no API keys, no gateway
...
multi-provider gateway (ADR-0010) and a queue of cross-vendor decisions (D1/D2/D4/D5,
Terra/Sol/MiniCheck ratifications). The user's call: development runs on Anthropic
exclusively, authenticated by the Claude Console OAuth profile (`ant auth login`,
ADR-0003) — no API keys, no non-Anthropic calls, no gateway in the runtime path.
...
- **Transport: direct Anthropic SDK only** (+ the first-class fake provider). The
  OpenClaw gateway's runtime role is deferred; OpenClaw remains dev-loop tooling.
...
- Fable 5 stays disqualified (retention/no-ZDR + price) — "Claude-only" does not
  rehabilitate it.
- Dissolved by collapse: D1, D2 (judge = direct SDK), D4, D5-for-now, R1, R3; R2
...
- **Keep the 4-model fleet and gateway.** Rejected by the user for this cycle: two
  vendors' worth of credentials, data-governance gates, and gateway hardening ahead of a
...
- `OpenClawProvider` and gateway Sessions 2–3 are not built this cycle.
...
## Sources
User decision 2026-09-02 (this session); ADR-0003/0009/0010; `docs/llm/dev-cycle.md`.
