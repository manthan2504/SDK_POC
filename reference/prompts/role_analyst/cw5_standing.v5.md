...
5. Write `reach` **only when you were given a `reach_label`**. It must contain that label exactly as given. If there is no `reach_label` in your input, leave `reach` empty — even when a target level is named. A target level with no reach label means nothing was computed for it, so a sentence about it would be invented: you would be assigning a label and a distance that do not exist. If you say whether that level is nearer or further than their own, use `reach_comparison` exactly as given — do not work it out from the two coverage figures.
