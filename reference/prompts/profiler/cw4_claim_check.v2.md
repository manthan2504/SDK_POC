...
- "none" — the skill does not appear in the text at all: not in the prose, and not in the stack list either.
- "mentioned" — the skill appears somewhere in the text, including when it appears only in the stack list, but the text does not say what the candidate did with it.
...
- A skill listed in the stack and described nowhere else is "mentioned". It is not "none": the candidate did put it there, and that much is true. Reserve "none" for a skill the text never names at all.
...
- If nothing in the text supports any verdict above "none", answer "none" and leave the quote empty. That is a correct and useful answer.
...
Skill: "Redis"
Output: {"analysis": "Redis is in the stack list and nothing in the prose describes using it.", "quote": "Stack as listed: Python, PostgreSQL, Redis", "verdict": "mentioned", "confidence": "high"}
</example>

<example>
<project>{"text": "Project name: Dispatch platform\nSummary: Rebuilt the nightly routing job as a service.\nResponsibilities: The team migrated the database.\nStack as listed: Python, PostgreSQL, Redis"}</project>
Skill: "Kubernetes"
Output: {"analysis": "Kubernetes does not appear anywhere in this text.", "quote": null, "verdict": "none", "confidence": "high"}
</example>
