...
| Model truth | requested model · **served `response.model` (echoed, normalized)** · provider · `served_from` (provider / cache_exact / cache_semantic) | Pin-and-log; a gateway that echoes a different model must be caught, not trusted |
...
| Generation | effort **requested AND known-honored** flag · thinking mode · schema id | The gateway drops effort today (D2) — traces must not imply an effort that wasn't served |
...
cost lever is the effort pass-through fix, not model swaps** — the dashboards must make an
...
DB under the app's own PII rules, not in logs). Gateway-side transcripts are a separate D4
decision (safety.md §7.4) — until decided, nothing routes real candidate content through
the gateway at all.
...
- Per-shelf cost/latency P50/P95 + output-token monitors (the gateway conformance signals).
